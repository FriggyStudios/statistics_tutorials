...
ggplot(data = RELAXATION) +
  geom_boxplot(mapping = aes(x = Treatment, y = Reduction)) +
  coord_flip()
```
...
```{r}
ggplot(data = BIODEX) +
  geom_boxplot(mapping = aes(x = Sex, y = IAS_Improvement)) +
  coord_flip() +
  facet_grid(. ~ Device)
```
...
Two-Sample T-Test and Confidence Interval: IAS_Improvement, Device 
```{r}
#t.test(BIODEX$IAS_Improvement,BIODEX$Device)
```
...
Plot a scatter plot of Body_Mass vs Grip_Strength with parallal lines model
```{r}
...
ggplot(mapping = aes(x = GRIP_STRENGTH_AND_MASS$Body_Mass, y = GRIP_STRENGTH_AND_MASS$Grip_Strength,color = GRIP_STRENGTH_AND_MASS$Climber_Type)) +
  ...
  stat_function(fun = function(x) -35 + 7.966 * x) +
   stat_function(fun = function(x) -90 + 7.966 * x)
```
